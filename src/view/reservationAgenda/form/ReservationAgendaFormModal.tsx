...
import ReservationAgendaForm from 'src/view/reservationAgenda/form/ReservationAgendaForm';
import ReservationAgendaService from 'src/modules/reservationAgenda/reservationAgendaService';

function ReservationAgendaFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ReservationAgendaService.create(data);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 rounded-t">
              <div className="text-lg font-semibold">
                {i18n('entities.reservationAgenda.new.title')}
              </div>
              <button
                type="button"
...
export default ReservationAgendaFormModal;
