...
import dealsSelectors from 'src/modules/deals/dealsSelectors';
import destroyActions from 'src/modules/deals/destroy/dealsDestroyActions';
import destroySelectors from 'src/modules/deals/destroy/dealsDestroySelectors';
...
function DealsListTable(props) {
  ...
  const hasPermissionToEdit = useSelector(
    dealsSelectors.selectPermissionToEdit,
  );
  const hasPermissionToDestroy = useSelector(
    dealsSelectors.selectPermissionToDestroy,
  );
            ...
            <tr>
              <TableColumnHeader>
                ...
                <TableColumnHeader
                  ...
                  name={'status'}
                  label={i18n(
                    'entities.deals.fields.status',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.deals.fields.customer',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'dateStart'}
                  label={i18n(
                    'entities.deals.fields.dateStart',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'dateEnded'}
                  label={i18n(
                    'entities.deals.fields.dateEnded',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.deals.fields.salesManagerResponsible',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.deals.fields.businessID',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.deals.fields.country',
                  )}
                />
              <TableColumnHeader />
            </tr>
              ...
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
                    {row.status
                      ? i18n(
                          `entities.deals.enumerators.status.${row.status}`,
                        )
                      : null}
                  </td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
                  ...
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">{row.dateEnded}</td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
                    <UserListItem value={row.salesManagerResponsible} />
                  </td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
