...
import PetPhotosForm from 'src/view/petPhotos/form/PetPhotosForm';
import PetPhotosService from 'src/modules/petPhotos/petPhotosService';

function PetPhotosFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await PetPhotosService.create(data);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 rounded-t">
              <div className="text-lg font-semibold">
                {i18n('entities.petPhotos.new.title')}
              </div>
              <button
                type="button"
...
export default PetPhotosFormModal;
