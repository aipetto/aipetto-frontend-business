...
function ProvidersToolbar(props) {
  const [
  ...
  const hasPermissionToCreate = useSelector(
    providersSelectors.selectPermissionToCreate,
  );
  const hasPermissionToImport = useSelector(
    providersSelectors.selectPermissionToImport,
  );
    ...
    if (disabled) {
      return (
        <span
          ...
          data-for="providers-list-toolbar-export-tooltip"
        >
          {button}
          <ReactTooltip id="providers-list-toolbar-export-tooltip" />
        </span>
      );
    }
    ...
    if (disabled) {
      return (
        <span
          ...
          data-tip-disable={!disabled}
          data-for="providers-list-toolbar-destroy-tooltip"
        >
          {button}
          <ReactTooltip id="providers-list-toolbar-destroy-tooltip" />
        </span>
      );
    }
  ...
  return (
    <div className="mb-4">
      ...
      {hasPermissionToAuditLogs && (
        <Link to="/audit-logs?entityNames=providers">
          <button
            className="mb-2 mr-2 text-sm disabled:opacity-50 disabled:cursor-default px-4 py-2 tracking-wide dark:border-gray-800 dark:bg-gray-800 dark:hover:bg-gray-600 dark:text-white text-gray-700 border border-gray-300 transition-colors duration-200 transform bg-white rounded-md hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
            type="button"
          >
