...
import contactsSelectors from 'src/modules/contacts/contactsSelectors';
import destroyActions from 'src/modules/contacts/destroy/contactsDestroyActions';
import destroySelectors from 'src/modules/contacts/destroy/contactsDestroySelectors';
...
function ContactsListTable(props) {
  ...
  const hasPermissionToEdit = useSelector(
    contactsSelectors.selectPermissionToEdit,
  );
  const hasPermissionToDestroy = useSelector(
    contactsSelectors.selectPermissionToDestroy,
  );
            ...
            <tr>
              <TableColumnHeader>
                ...
                <TableColumnHeader
                  ...
                  name={'firstName'}
                  label={i18n(
                    'entities.contacts.fields.firstName',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'lastName'}
                  label={i18n(
                    'entities.contacts.fields.lastName',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.contacts.fields.country',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'companyName'}
                  label={i18n(
                    'entities.contacts.fields.companyName',
                  )}
                />
              <TableColumnHeader />
            </tr>
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">{row.firstName}</td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">{row.lastName}</td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
                    <CountryListItem value={row.country} />
                  </td>
                  <td className="whitespace-nowrap px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">{row.companyName}</td>
                  <td
